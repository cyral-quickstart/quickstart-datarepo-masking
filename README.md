...
| --------------- |---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------| --------------- |
| [Consistent Format Preserve](./masking-examples/consistent-format-preserve) | UDFs that replace the field's contents with semi-randomized values. It is noteworthy that this transformation maintains consistency, ensuring that identical field content will consistently translate into the same semi-randomized value. This predictable behavior facilitates reliable data manipulation and analysis within the specified context. There are currently two UDFs - one that uses a pseudorandom number generator to randomly generate characters (`consistent_mask`), and another which uses a SHA-256 hash of the value to generate pseudorandom numbers (`consistent_mask_hash`). The former is likely to be more performant but depends on the underlying engine's PRNG algorithm to generate values, while the later is designed to be portable and consistent across database engines and tools. | :white_check_mark: PostgreSQL <br> :white_check_mark: SQL Server <br> :white_check_mark: Oracle <br> :white_check_mark: MySQL <br> :white_check_mark: Snowflake |
